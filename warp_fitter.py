...
comparison_data = {
    ...
    "DM Tau": {
        ...
        "NIR_ulim": True,
        "alpha_S": 2.19,
        "alpha_S_err_low": 1.21,
        "alpha_S_err_high": 2.72,
    },
    "HD 135344B": {
        "log_Mdot": -8.0,
        "NAI": 0.405,
    ...
    "HD 34282": {
        ...
        "NIR_ulim": False,
        "alpha_S": 3.86,
        "alpha_S_err_low": 2.14,
        "alpha_S_err_high": 4.79,
    },
    "J1604": {
        "log_Mdot": -10.5,
        "NAI": 0.059,
    ...
    "J1615": {
        ...
        "NIR_ulim": True,
        "alpha_S": 1.20,
        "alpha_S_err_low": 0.66,
        "alpha_S_err_high": 1.48,
    },
    "J1842": {
        "log_Mdot": -8.8,
        "NAI": 0.074,
        ...
        "NIR_ulim": False,
        "alpha_S": 0.403,
        "alpha_S_err_low": 0.223,
        "alpha_S_err_high": 0.499,
    },
    "J1852": {
        "log_Mdot": -8.7,
        "NAI": 0.024,
        ...
        "NIR_ulim": True,
        "alpha_S": 0.362,
        "alpha_S_err_low": 0.201,
        "alpha_S_err_high": 0.449,
    },
    "LkCa15": {
        "log_Mdot": -8.4,
        "NAI": 0.053,
        ...
        "NIR_ulim": False,
        "alpha_S": 0.174,
        "alpha_S_err_low": 0.097,
        "alpha_S_err_high": 0.216,
    },
    "MWC 758": {
        "log_Mdot": -7.15,
        "NAI": 0.429,
    ...
    "PDS 66": {
        ...
        "NIR_ulim": False,
        "alpha_S": 0.0806,
        "alpha_S_err_low": 0.0446,
        "alpha_S_err_high": 0.0999,
    },
    "SY Cha": {
        "log_Mdot": -9.2,
        "NAI": 0.075,
        ...
        "NIR_ulim": False,
        "alpha_S": 0.0166,
        "alpha_S_err_low": 0.0092,
        "alpha_S_err_high": 0.0205,
    },
    "V4046 Sgr": {
        "log_Mdot": -9.3,
        "NAI": 0.030,
        ...
        "NIR_ulim": True,
        "alpha_S": 0.111,
        "alpha_S_err_low": 0.061,
        "alpha_S_err_high": 0.138,
    },
}
...
def plot_velocity_map(dv_grid, radii, phi_highres, meta_params, fname):
    ...
    pcm = ax.contourf(X, Y, dv_grid, cmap="RdBu_r", levels=dv_levels)
    ...
    for r in np.arange(dr, Rmax + dr, dr):
        circle = plt.Circle(
            (0, 0), r, color="k", ls=":", lw=0.5, fill=False, zorder=10,
        )
        ax.add_patch(circle)
        ax.text(
            r / np.sqrt(2),
...
def grid_up(fname_full, phi_highres=np.linspace(-np.pi, np.pi, 257), **kwargs):
    ...
    if (
        not os.path.isfile(f"{fname}_gridded.npy")
        or not os.path.isfile(f"{fname}_rgrid.npy")
        or not os.path.isfile(f"{fname}_phigrid.npy")
        or hreset
    ):
        radii, dv_grid = mu.interpolate_to_uniform_azimuth(
            fname_full, phi_highres=phi_highres, R_trunc=trunc_rout
        )
    ...
    if not os.path.isfile(f"{fname}_meta_params.npy") or hreset:
        meta_params = mu.write_meta(
            radii,
            dv_grid,
            trunc_rout=trunc_rout,
            dist_pc=dist_pc,
            beam_fwhm_arcsec=beam_fwhm_arcsec,
            mstar_norm=mstar_norm,
        ...
        if meta_params.get("channel_spacing", None) != channel_spacing:
            print(
                f"Channel spacing mismatch: {meta_params.get('channel_spacing')} vs {channel_spacing}. Updating and marking for rescore..."
            )
    ...
    if not os.path.isfile(f"{fname}_rescaled.npy") or hreset:
        dv_rescaled = (
            dv_grid  # DO NOT RESCALE rescale_vfield(radii, dv_grid, meta_params)
        )
...
def save_gp_samples_to_csv(
    samples, r_eval, discname, quantity_name, output_dir="gp_samples"
):
    """
...
def fit_gp_from_warp_model(
    ...
    for i, r in enumerate(rgrid):
        if r >= r0 + beam_frac * beam_radius_au:
            ...
            nbeams = 2.0 * np.pi * r / beam_radius_au
    ...
    r_fit = np.arange(r_min, r_max + beam_radius_au / 2.0, beam_radius_au / 2.0)
    ...
    interp_i = interp1d(
        ...
        delta_i_obs,
        bounds_error=False,
        fill_value=(delta_i_obs[0], delta_i_obs[-1]),
    )
    interp_pa = interp1d(
        rgrid,
        delta_pa_obs,
        bounds_error=False,
        fill_value=(delta_pa_obs[0], delta_pa_obs[-1]),
    )
    interp_i_err = interp1d(
        rgrid,
        delta_i_err,
        bounds_error=False,
        fill_value=(delta_i_err[0], delta_i_err[-1]),
    )
    interp_pa_err = interp1d(
        rgrid,
        delta_pa_err,
        bounds_error=False,
        fill_value=(delta_pa_err[0], delta_pa_err[-1]),
    )
    ...
    kernel_i = C(1.0) * Matern(
        length_scale=beam_radius_au * 2.0, nu=2.5
    )  # + WhiteKernel(noise_level=i_err)
    kernel_pa = C(1.0) * Matern(
        length_scale=beam_radius_au * 2.0, nu=2.5
    )  # + WhiteKernel(noise_level=pa_err)
    ...
    gp_i = GaussianProcessRegressor(
        kernel=kernel_i, alpha=alpha_i, normalize_y=False, n_restarts_optimizer=10
    )
    gp_pa = GaussianProcessRegressor(
        kernel=kernel_pa, alpha=alpha_pa, normalize_y=False, n_restarts_optimizer=10
    )
    ...
    psi_ravg_samples = np.trapz(log_psi_samples, r_eval[:, None], axis=0) / (
        r_eval[-1] - r_eval[0]
    )
    psi_ravg = np.mean(psi_ravg_samples)
    psi_ravg_std = np.std(psi_ravg_samples)
    log_theta_samples = np.log10(theta_samples)
    theta_ravg_samples = np.trapz(log_theta_samples, r_eval[:, None], axis=0) / (
        r_eval[-1] - r_eval[0]
    )
    ...
    gamma_std = np.std(
        np.arctan2(i_samples, -pa_samples * np.sin(incl)), axis=1
    )  # shape: (r_eval,)
...
def plot_combined_velocity_and_profiles(
    ...
    **kwargs,
    ...
    delta_i = interp1d(
        fit_result["r_eval"],
        fit_result["delta_i_mean"],
        bounds_error=False,
        fill_value=0.0,
    )(rgrid)
    delta_pa = interp1d(
        fit_result["r_eval"],
        fit_result["delta_pa_mean"],
        bounds_error=False,
        fill_value=0.0,
    )(rgrid)
    ...
    dv_obs_xy = map_coordinates(dv_grid, coords, order=1, mode="nearest").reshape(
        Nxy, Nxy
    )
    dv_model_xy = map_coordinates(model_grid, coords, order=1, mode="nearest").reshape(
        Nxy, Nxy
    )
    ...
    theta_samples = np.sqrt(
        i_samples ** 2 + (np.sin(incl) * pa_samples) ** 2
    )  # shape (n_r, n_samples)
    ...
    dpa_dlnr_samples = np.gradient(pa_samples, ln_r_eval, axis=0)
    psi_samples = np.sqrt(
        di_dlnr_samples ** 2 + (np.sin(incl) * dpa_dlnr_samples) ** 2
    )  # shape (n_r, n_samples)
    ...
    gamma_samples = (
        np.arctan2(i_samples, -pa_samples * np.sin(incl)) + np.pi
    )  # Wrap to [0, pi] for consistency
    # gamma_samples = np.arctan2(pa_samples * np.sin(incl), i_samples)+ np.pi  # Wrap to [0, pi] for consistency

    # Wrap gamma into [-pi/2, pi/2]
    gamma_samples_wrapped = (gamma_samples + 2 * np.pi) % (
        2 * np.pi
    ) - np.pi  # Now in [0, 2π)
    ...
    if show_residual:
        ...
        width_ratios = [
            ...
            1,
            0.05,
        ]  # colorbar | observed | model | residual | colorbar
        ncols = 4
    ...
    for ax in [ax_obs, ax_mod]:
        ...
        for r in radii_au:
            circle = plt.Circle(
                (0, 0), r, color="k", ls=":", lw=0.5, fill=False, zorder=20
            )
            ax.add_patch(circle)
            ax.text(
                r / np.sqrt(2),
        ...
        for angle in angles_deg:
            ...
            ax.plot(
                [0, Rmax * np.cos(angle_rad)],
                [0, Rmax * np.sin(angle_rad)],
                color="k",
                ls=":",
                lw=0.5,
                zorder=20,
    ...
    ax2.errorbar(
        fit_result["r_fit"],
        np.rad2deg(fit_result["delta_i_fit"]),
        yerr=np.rad2deg(fit_result["delta_i_err"]),
        fmt="o",
        alpha=0.5,
        zorder=2,
    )
    ...
    ax3.errorbar(
        fit_result["r_fit"],
        np.rad2deg(fit_result["delta_pa_fit"]),
        yerr=np.rad2deg(fit_result["delta_pa_err"]),
        fmt="o",
        alpha=0.5,
        zorder=2,
    )
    ...
    theta_max_per_sample = np.max(
        np.rad2deg(theta_samples), axis=0
    )  # shape (n_samples,)
    ...
    # Print result
    print(
        f"Max β (median ±1σ): {theta_max_median:.2f}° (+{theta_max_upper - theta_max_median:.2f}, -{theta_max_median - theta_max_lower:.2f})"
    )
...
def plot_gp_three_panel_comparison(dv_obs, R, PHI, vphi_fn, vr_fn, vz_fn, incl_rad):
    ...
    im0 = ax0.contourf(
        np.rad2deg(PHI), R, dv_obs, levels=levels, cmap="RdBu_r", extend="both"
    )
    ...
    im1 = ax1.contourf(
        np.rad2deg(PHI), R, dv_fit, levels=levels, cmap="RdBu_r", extend="both"
    )
    ...
    # Panel 3: Residuals
    im2 = ax2.contourf(
        np.rad2deg(PHI), R, dv_residual, levels=levels, cmap="RdBu_r", extend="both"
    )
...
def permutation_corr(x, y, n_permutations=10000, seed=42):
    ...
    permuted_rs = np.array(
        [np.corrcoef(x, rng.permutation(y))[0, 1] for _ in range(n_permutations)]
    )
...
def compare_warp_to_curone(
    ...
    print(
        f"{'Disc':<10} {'i range (deg)':<20} {'PA range (deg)':<20} {'Warp amplitude (deg)':<20} {'A-index':<10} {'log(Mdot)':<10}"
    )
    print("=" * 70)
    ...
    for label, result in results_dict.items():
        ...
        warp_amplitude = (
            dtheta  # np.sqrt(delta_i_mean**2+(np.sin(i0)**2) * delta_pa_mean**2)
        )
        ...
        print(
            f"{long_name:<10} {i_min:.2f}–{i_max:.2f}     {pa_min:.2f}–{pa_max:.2f}     {warp_amplitude:.2f}$\pm${warp_amplitude_err:.2f}    {comp['NAI']:<10.2f} {comp['log_Mdot']:<10.2f}"
        )
    ...
    print("\\toprule")
    print(
        "Disc & $M_\\star$ [$M_\\odot$] & $i_0$ [$^\circ$] & $R_\\mathrm{out}$ [au] & $\delta i_{\mathrm{min/max}}$ [$^\circ$] & $\delta$PA$_{\mathrm{min/max}}$  [$^\circ$] & $\\beta_\mathrm{max}$ [$^\circ$] & NAI & $\\log \\dot{M}_\mathrm{acc}$ [$M_\odot$~yr$^{-1}$] & $\langle \log \psi \\rangle_R$ DB?\\\\"
    )
    print("\\midrule")
    ...
    for label, result in results_dict.items():
        ...
        print(
            f"{discname} & {M_star:.2f} & {np.rad2deg(incl):.1f} & {rout:.1f} & {i_min:.1f}--{i_max:.1f} & {pa_min:.1f}--{pa_max:.1f} & {warp_amplitude:.2f}$\pm${warp_amplitude_err:.2f} &{log_psi:.2f}$\pm${log_psi_err:.2f} & {comp['NAI']:.2f} & {comp['log_Mdot']:.2f} & {db_bool:s}\\\\"
        )
        i += 1
    print("\\bottomrule")
    print("\\end{tabular}")
    ...
    if NIR_exc:
        ...
        axes[0].errorbar(
            xvals[mask_incs],
            non_axisym[mask_incs],
            xerr=xerrs[mask_incs],
            fmt="o",
            color="tab:red",
            ecolor="gray",
            elinewidth=1,
        ...
        for i, txt in enumerate(labels):
            if mask_incs[i]:
                axes[0].annotate(
                    txt, (xvals[i], non_axisym[i]), ha="center", fontsize=9
                )
        ...
        if inc_dbell:
            annotate_correlations(
                axes[0], xvals, non_axisym, label="All", ha="right", va="bottom"
            )
            mask = [lbl not in double_bell_labels for lbl in labels]
            annotate_correlations(
                axes[0],
                np.array(xvals)[mask],
                np.array(non_axisym)[mask],
                color="red",
                label="No DBell",
                ha="right",
                va="center",
        ...
        if inc_dbell:
            ...
            annotate_correlations(
                axes[1],
                np.array(xvals)[mask & mask_incs],
                np.array(log_Mdots)[mask & mask_incs],
                color="red",
                label="No DBell",
                ha="right",
                va="center",
        ...
        for i, txt in enumerate(labels):
            if mask_incs[i]:
                axes[2].annotate(
                    txt, (xvals[i], log_Mdot_norm[i]), ha="center", fontsize=9
                )
        axes[2].set_ylabel(
            "$\log(\\dot{{M}}_\mathrm{acc} / M_*^2)$ [$M_\\odot^{-1}$ yr$^{-1}$]"
        )
        if inc_dbell:
            annotate_correlations(
                axes[2],
            ...
            annotate_correlations(
                axes[2],
                np.array(xvals)[mask & mask_incs],
                np.array(log_Mdot_norm)[mask & mask_incs],
                color="red",
                label="No DBell",
                ha="right",
                va="center",
        ...
        if inc_dbell:
            ...
            annotate_correlations(
                axes[3],
                np.array(xvals)[mask & mask_incs],
                np.array(nir_excess)[mask & mask_incs],
                color="red",
                label="No DBell",
                ha="right",
                va="bottom",
        ...
        plt.tight_layout()
        plt.savefig(
            "correlations_figure_with_nir.pdf", bbox_inches="tight", format="pdf"
        )
        plt.show()
        ...
        if inc_dbell:
            annotate_correlations(
                axes[0], xvals, non_axisym, label="All", ha="right", va="top"
            )
            mask = [lbl not in double_bell_labels for lbl in labels]
            annotate_correlations(
                axes[0],
        ...
        if inc_dbell:
            annotate_correlations(
                axes[1], xvals[mask_incs], log_Mdots[mask_incs], label="All"
            )
            annotate_correlations(
                axes[1],
                np.array(xvals)[mask & mask_incs],
            ...
            annotate_correlations(
                axes[1], xvals[mask_incs], log_Mdots[mask_incs], label=None
            )
        ...
        if inc_dbell:
            annotate_correlations(
                axes[2], xvals[mask_incs], log_Mdot_norm[mask_incs], label="All"
            )
            annotate_correlations(
                axes[2],
                np.array(xvals)[mask & mask_incs],
            ...
            annotate_correlations(
                axes[2], xvals[mask_incs], log_Mdot_norm[mask_incs], label=None
            )
    ...
    for i, label in enumerate(labels):
        ...
        if not logspace:
            if label == "J1615":
                txt = ax.annotate(
                    label,
                    (np.absolute(delta_pa[i]) - 0.1, np.absolute(delta_i[i]) - 0.3),
                    ha="center",
                    fontsize=9,
                )
            elif label == "PDS 66":
                ...
                txt = ax.annotate(
                    label,
                    (np.absolute(delta_pa[i]) - 0.1, np.absolute(delta_i[i]) + 0.15),
                    ha="center",
                    fontsize=9,
                )
            ...
            txt = ax.annotate(
                label,
                (np.absolute(delta_pa[i]) * 0.9, np.absolute(delta_i[i]) * 1.1),
                ha="center",
                fontsize=9,
            )
        texts.append(txt)
...
def plot_inclination_vs_pa_compare(
    results_12co, results_13co, labels_12co="12CO", labels_13co="13CO"
):
    def extract_data(results_dict):
        ...
        return (
            ...
            np.array(delta_i),
            np.array(delta_pa),
            np.array(delta_i_errs),
            np.array(delta_pa_errs),
            np.array(incls),
        )
...
def plot_warp_amplitude_comparison(results_12co, results_13co):
    ...
    def compute_amplitude(result):
        ...
        amp_err = (
            np.sqrt((di * di_err) ** 2 + ((np.sin(incl) ** 2 * dpa * dpa_err) ** 2))
            / amp
        )
    ...
    base_labels_13co = {
        label.replace("_13co", "").replace("_dbell", "") for label in results_13co
    }
    ...
    # Scatter points
    sc = ax.scatter(
        x, y, c=colors, cmap="cool", edgecolors="k", s=60, zorder=2, vmin=0.0, vmax=60.0
    )
...
def plot_beam_amplitude_comparison(
    results_nominal, results_b0p30, double_bell_labels=None
):
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    shared = sorted(
        set(label.replace("_b0p30", "") for label in results_b0p30)
        & set(results_nominal)
    )
...
def plot_warp_vs_mdot_norm(results_dict, disc_name_map, inc_dbell=True):
    ...
    for label, result in results_dict.items():
        ...
        if (
            not np.isfinite(psi_ravg)
            or not np.isfinite(psi_ravg_std)
            or not np.isfinite(log_mdot)
            or not np.isfinite(M_star)
        ):
            continue
    ...
    if not inc_dbell:
        ...
        annotate_correlations(
            ax, xvals[~is_dbell], yvals[~is_dbell], label="No DBell", color="tab:red"
        )
...
def plot_warp_vs_alphaS(results_dict, disc_name_map, inc_dbell=True):
    ...
    for label, result in results_dict.items():
        ...
        if (
            "alpha_S" not in comp
            or "alpha_S_err_low" not in comp
            or "alpha_S_err_high" not in comp
        ):
            continue
        ...
        if not np.all(
            np.isfinite([psi_ravg, psi_ravg_std, alpha_S, err_low, err_high])
        ):
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--Nbeam", type=int, default=1, help="Number of beams to sample"
    )
    ...
    parser.add_argument(
        "--warp",
        action="store_true",
        help="Run warp analysis with GP fit to inclination profile",
    )
    parser.add_argument(
        "--co13", action="store_true", help="Use 13CO directory versions and filenames"
    )
    parser.add_argument(
        "--clip",
        type=float,
        help="Clip without scaling (just in warp analysis for now)",
    )
    parser.add_argument(
        "--unclip", action="store_true", help="Unclip -- overrides clip"
    )
    parser.add_argument("--grid", action="store_true", help="Grid the txt files")
    parser.add_argument(
        "--isocomp",
        action="store_true",
        help="Compare Delta i and Delta PA between 12CO and 13CO",
    )
    parser.add_argument(
        "--beamcomp",
        action="store_true",
        help="Compare warp amplitudes for different beam sizes (e.g. b0p30 vs default)",
    )
    parser.add_argument(
        "--residual", action="store_true", help="Show residuals of the velocity field"
    )
    parser.add_argument(
        "--wcomp", action="store_true", help="Compare warp fits between targets"
    )
    ...
    if "testaxi" in target_list:
        ...
        for full_path in axi_files:
            fname = os.path.basename(full_path)
            match = re.search(
                r"axisym_inc_(\d+)_vr_([0-9.]+)_vz_([0-9.]+)_vphi_([0-9.]+)\.txt$",
                fname,
            )
            if match:
    ...
    for (
        ...
        print(
            f"  - {label:<20}  file: {fname:<55}  dist: {dist:>5} pc  beam: {beam}''  M⋆: {mstar} M☉  i: {incl}"
        )
    print()
    ...
    for (
        ...
        if args.warp:
            ...
            if label in disc_name_map and disc_name_map[label] in comparison_data:
                ...
                results_warp[label]["meta_params"] = np.load(
                    folder + "/" + meta_path, allow_pickle=True
                ).item()
            else:
        ...
        for (
            ...
            for molecule_label, use_13co in [("12co", False), ("13co", True)]:
                ...
                if fit_result is not None and label in disc_name_map:
                    ...
                    fit_result["meta_params"] = np.load(
                        folder + "/" + meta_path, allow_pickle=True
                    ).item()
            ...
            if os.path.exists(os.path.join(folder_030, fname_030)):
                ...
                if result_030 is not None:
                    ...
                    if os.path.exists(meta_030_path):
                        result_030["meta_params"] = np.load(
                            meta_030_path, allow_pickle=True
                        ).item()
            ...
            if os.path.exists(os.path.join(folder_015, fname_015)):
                ...
                if result_015 is not None:
                    ...
                    if os.path.exists(meta_015_path):
                        result_015["meta_params"] = np.load(
                            meta_015_path, allow_pickle=True
                        ).item()
